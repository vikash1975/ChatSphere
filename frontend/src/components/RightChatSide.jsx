...
import { ChevronLeftIcon, PaperAirplaneIcon, PhoneIcon, VideoCameraIcon, SparklesIcon } from '@heroicons/react/24/outline';
...
const ChatArea = ({ selectedUser, onBack }) => {
  ...
  const { messages = [], loading = false } = chatState;
  
  const [message, setMessage] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const messagesEndRef = useRef(null);
  const typingTimeoutRef = useRef(null); //  NEW: Debounce ref
  ...
  //  FIXED: New message with duplicate check
  useEffect(() => {
  ...
  }, [dispatch, scrollToBottom, messages]);
  ...
  useEffect(() => {
    const handleTyping = ({ senderId }) => {
      console.log(" Typing from:", senderId, "Selected:", selectedUser?._id);
      ...
      <div className="p-6 border-b border-white/20 bg-white/5 backdrop-blur-xl sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
           ...
           {isTyping && (         <div className="flex items-center space-x-2 mt-1 p-1 bg-black/20 rounded-lg">
...
const MessageBubble = ({ msg, isOwn }) => {
  return (
    <div className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-4`}>
      <div className={`max-w-md px-5 py-3 rounded-3xl ${
        isOwn 
...
const NoChatSelected = () => (
  <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
    ...
    <h2 className="text-4xl text-gray-500 font-bold bg-gradient-to-r from-white to-purple-200 bg-clip-text mb-4">
